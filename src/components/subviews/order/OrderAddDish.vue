<template>
  ...
  <button
    @click="showModal"
    class="flex justify-center items-end gap-x-2 text-white bg-gray-800 hover:bg-gray-600 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-full md:w-fit py-2.5 px-12 text-center"
    type="button"
  >
    <svg
      class="w-[18px] h-[18px]"
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        >
          <button
            @click="submitForm"
            type="button"
            class="text-white w-full bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
          >
            Thêm
          </button>
          <button
            @click="closeModal"
            type="button"
            class="py-2.5 px-5 w-full ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
          >
            Huỷ
          </button>
        </div>
...
const submitForm = async () => {
  ...
  try {
    ...
    toast.success('Thêm vào đơn món thành công')
    closeModal()
  } catch (error) {
    console.error('Error submitting form:', error)
    toast.error('Thêm vào đơn món thất bại')
    closeModal()
  }
}
</script>
