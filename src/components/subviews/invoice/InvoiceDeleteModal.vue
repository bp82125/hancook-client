  ...
  >
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
        ...
        >
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Xoá chi tiêu</h3>
          <button
            @click="closeModal"
            id="closeModal"
...
onMounted(() => {
  const $modalElement = document.querySelector('#deleteInvoiceModal')
  const modalOptions = {
    backdrop: 'static',
...
const deleteInvoice = async () => {
  const response = await invoiceStore.deleteInvoice(id.value)
