      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        >
          <button
            @click="submitForm"
            type="button"
            class="text-white justify-center w-full bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          >
            Thêm
          </button>
          <button
...
let modal
onMounted(() => {
  const $modalElement = document.querySelector('#addDishCartModal')
  const modalOptions = {
    backdrop: 'static',
...
const items = computed(() => {
  return cartItemStore.order
})

const dishes = computed(() => {
  return dishStore.dishes.filter(
    (dish) => !items.value.details.some((detail) => detail.dish.id === dish.id)
  )
})
